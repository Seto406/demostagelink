...
## 2025-05-25 - Core Interaction Tooltips
**Learning:** Frequently used core interaction buttons like `LikeButton` and `BookmarkButton` were missing tooltips. Adding them ensures that users clearly understand their actions, and drastically improves accessibility.
**Action:** Always wrap primary action icon-only buttons with `Tooltip` and `TooltipProvider`, not just auxiliary buttons.
