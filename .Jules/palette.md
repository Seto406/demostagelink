...
## 2025-05-25 - Custom Component Focus States
**Learning:** Custom interactive elements (like `motion.button` wrappers in `LikeButton` and `BookmarkButton`) often lack default browser focus indicators, making them invisible to keyboard-only users.
**Action:** Always manually apply the application's standard focus rings (`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary/50 focus-visible:ring-offset-2`) to all interactive elements lacking default styles to maintain accessible keyboard navigation.
