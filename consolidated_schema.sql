...
CREATE TABLE IF NOT EXISTS public.payments (
    ...
    paymongo_checkout_id TEXT, -- Made nullable for manual payments
    description TEXT,
    payment_method TEXT DEFAULT 'paymongo', -- Added for manual payments
    proof_of_payment_url TEXT, -- Added for manual payments
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
-- system_settings (Added from manual payments)
CREATE TABLE IF NOT EXISTS public.system_settings (
    key TEXT PRIMARY KEY,
    value TEXT, -- Changed to allow NULL in later migrations, but keeping consistent
    description TEXT,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
-- Collaboration Logs RLS
DROP POLICY IF EXISTS "Sender can view own sent logs" ON public.collaboration_logs;
...
DROP POLICY IF EXISTS "Recipient can view logs sent to them" ON public.collaboration_logs;
...
DROP POLICY IF EXISTS "Admins can view all logs" ON public.collaboration_logs;
...
CREATE POLICY "Authenticated users can upload their own avatar" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'avatars' AND auth.uid()::text = (storage.foldername(name))[1]);
...
CREATE POLICY "Authenticated users can upload posters" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'posters');

DROP POLICY IF EXISTS "Users can update own posters" ON storage.objects;
CREATE POLICY "Users can update own posters" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'posters' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Users can delete own posters" ON storage.objects;
CREATE POLICY "Users can delete own posters" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'posters' AND auth.uid()::text = (storage.foldername(name))[1]);
...
CREATE POLICY "Anyone can upload payment proofs" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'payment_proofs');

DROP POLICY IF EXISTS "Admin view all proofs" ON storage.objects;
CREATE POLICY "Admin view all proofs" ON storage.objects FOR SELECT USING (bucket_id = 'payment_proofs' AND EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "Users view own proofs" ON storage.objects;
CREATE POLICY "Users view own proofs" ON storage.objects FOR SELECT USING (bucket_id = 'payment_proofs' AND auth.uid() = owner);

-- System Assets
INSERT INTO storage.buckets (id, name, public) VALUES ('system_assets', 'system_assets', true) ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS "Public Access System Assets" ON storage.objects;
CREATE POLICY "Public Access System Assets" ON storage.objects FOR SELECT USING (bucket_id = 'system_assets');

DROP POLICY IF EXISTS "Admin Upload System Assets" ON storage.objects;
CREATE POLICY "Admin Upload System Assets" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'system_assets' AND EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "Admin Update System Assets" ON storage.objects;
CREATE POLICY "Admin Update System Assets" ON storage.objects FOR UPDATE USING (bucket_id = 'system_assets' AND EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "Admin Delete System Assets" ON storage.objects;
CREATE POLICY "Admin Delete System Assets" ON storage.objects FOR DELETE USING (bucket_id = 'system_assets' AND EXISTS (SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role = 'admin'));
