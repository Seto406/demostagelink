...
BEGIN
    ...
    SET
        ...
        date = CASE WHEN show_changes ? 'date' THEN (show_changes->>'date')::date ELSE date END,
        show_time = CASE WHEN show_changes ? 'show_time' THEN (show_changes->>'show_time')::timestamptz ELSE show_time END,
        ...
        -- JSON columns need to be extracted as JSONB, not text (-> vs ->>)
        ...
        -- Array handling: extract as text, convert to array
        tags = CASE
