...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (userError || !userData?.user?.email) {
      ...
      // We return success=false but 200 to avoid client-side errors if user doesn't exist (e.g. deleted user)
      // Or throw error if critical? Let's throw error for now to catch issues.
