...
export function ProductionModal({ open, onOpenChange, showToEdit, onSuccess }: ProductionModalProps & { showToEdit?: Tables<"shows"> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPro && !showToEdit) {
      // Check for Basic Tier limit (2 total shows)
      ...
      if (count !== null && count >= 2) {
        toast({
          title: "Limit Reached",
          description: "Basic Plan limit reached. You can only have up to 2 shows.",
          variant: "destructive",
        });
        setUpsellContext({
          featureName: "Unlimited Shows",
          description: "Upgrade to Premium to post more than 2 shows.",
        });
        setUpsellOpen(true);
        return;
      }
